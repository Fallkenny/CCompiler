...
int Init_declarator(){
	...
	else if(tk == tkIDENTIFIER){// identifier
		getToken();
		return 1;
	}
	else{return 0;}
...
//Initializer_list1Hash -> , Designation Initializer Initializer_list1Hash | , Initializer Initializer_list1Hash | ? 
int Initializer_list1Hash(){
	if(tk == tkCOMMA){// ,
		getToken(); 
		if (Designation()){
